function [ best_found, processing_time, evaluation_number ] = Generational_GA( test_function, population_size, chromosome_size, generation_number, tournament_size, crossover_rate, mutation_rate, selection_type, crossover_type, termination_condition, max_evaluations )
%The function to run the canonical generational genetic algorithm
%   Outputs:
...
next_ga_population=zeros(population_size,chromosome_size);
next_ga_fitness=zeros(population_size,1);

%starting the main loop
for generation=1:generation_number
    for chromosome_counter=1:2:population_size
        ...
        %Saving in the new generation
        next_ga_population( chromosome_counter, : ) = child_1;
        next_ga_population( chromosome_counter + 1, : ) = child_2;
        next_ga_fitness( chromosome_counter ) = child_1_fitness;
        next_ga_fitness( chromosome_counter + 1 ) = child_2_fitness;
    ...
    %Replacing the new generation on the previous one
    ga_population = next_ga_population;
    ga_fitness = next_ga_fitness;
