function [ best_found, processing_time, evaluation_number ] = Dual_Population_GA_DeleteOldest( test_function, population_size, chromosome_size, iteration_number, tournament_size, crossover_rate, mutation_rate, selection_type, crossover_type, termination_condition, max_evaluations )
...
if nargin<11
    ...
    if nargin<10
        termination_condition = 'Convergence'; % 'Convergence' or 'No-Improvement' or 'Max-Evaluations' or 'Fixed'
        if nargin<9
...
%enforcing even population sie
if mod(population_size, 2) == 1
...
for iteration=1:iteration_number
    ...
    if mod(iteration_number, 2) == 1
        ga_population_1( individual_counter, : ) = child_1;
        ga_population_1( individual_counter + 1, : ) = child_2;
        ga_fitness_1( individual_counter ) = child_1_fitness;
        ga_fitness_1( individual_counter + 1 ) = child_2_fitness;
    else        
        ga_population_2( individual_counter, : ) = child_1;
        ga_population_2( individual_counter + 1, : ) = child_2;
        ga_fitness_2( individual_counter ) = child_1_fitness;
        ga_fitness_2( individual_counter + 1 ) = child_2_fitness;
    end
    
    individual_counter = individual_counter + 2;
    if individual_counter > population_size
        individual_counter = individual_counter - population_size;
    end
